...
test('Form state is retained when minimizing a form in the workspace', async ({ page }) => {
  ...
  await test.step('And I fill the `Neutrophils` result as `3500`', async () => {
    await page.locator('#ManualEntryNeutrophilsMicroscopic').fill('3500');
  });
  ...
  await test.step('Then I should see the entered data retained in the form', async () => {
    ...
    await expect(neutrophils).toHaveValue('3500');
  });
